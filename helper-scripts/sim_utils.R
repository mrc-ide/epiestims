...
##' @param p_report probability of cases being reported
##' @return matrix of
...
simulate_incidence <- function(incid_init, nlocations,
  ...
  # For underreporting, new array adjusted by p_report
  reported <- array(
    NA, dim = c(ndays + init_days, nlocations, nvariants)
  )
  for (day in seq_len(ndays + init_days)) {
    for (loc in seq_len(nlocations)) {
      for (v in seq_len(nvariants)) {
        reported[day,loc,v] <- rbinom(1,size =incid[day,loc,v],prob = p_report)
      }
    }
  }
  reported
...
# Wrapper for simulating incidence with a changing epsilon
simulate_varyeps_incid_wrapper <- function(rt_ref, epsilon, si, incid_init,
                                   n_loc = 1, n_v = 2,
                                   ndays = 100, nsims = 100,
                                   p_report = 1, ...) {
  ## having this as 20 and starting with 1 case of the variant can lead to an infinite loop
