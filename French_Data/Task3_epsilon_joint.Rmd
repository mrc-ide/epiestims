...
incid <- array(NA, dim = c(nrow(I_UK2[[1]]), 
                           ncol(I_UK2[[1]]) - 1,
                           length(I_UK2)))
for(i in 1:length(I_UK2)){
  incid[,,i] <- as.matrix(I_UK2[[i]][,-1])
}

## start estimating at 3 weeks
t_min <- 14L
t_max <- seq(from = 21, to = nrow(I_UK2[[1]]), by = 7)
...
estimates <- map(
  t_max, function(tmax) {
      estimate_joint(
        incid = incid,
        si_distr = cbind_rep(x = Epi_param$SI, n = length(I_UK2)), 
        mcmc_control = Epi_param$mcmc_control,
        priors = priors,
        t_min = t_min,
...
names(estimates) <- I_UK2[[1]][["date"]][t_max]
...
ggsave("uk_delta_over_alpha.pdf", p)
...
p <- ggplot(eps_estimates) +
  geom_point(aes(date, `50%`)) +
  geom_linerange(aes(x = date, ymin = `2.5%`, ymax = `97.5%`)) +
  facet_wrap(~`Leave out`, scales = "free_y") +
  geom_hline(
    yintercept = 1, linetype = "dashed"
  ) +
  ylab("Transmission Advantage") +
...
ggsave("UK_leave_one_region_out.pdf", p)
