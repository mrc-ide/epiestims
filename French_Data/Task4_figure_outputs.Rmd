...
errb <- function(d1,d2,title){
  ...
  d$methods <- rep(c('joint','naive'),each=nrow(d1))
  ...
  p <- ggplot(d, aes(x=med, y=x , fill=methods, color=methods)) + 
    ...
    ggtitle(title) +
    geom_vline(xintercept=c(1), linetype="dashed",color='red3') +
    geom_hline(yintercept = c(4.5,nrow(d1)-4+3.5), linetype="dashed", color='blue3')+
...
heatplot<- function(x,dealing_with,xlab,ylab,title,epsilon){
  ...
  p <- ggplot(x, aes(x,y)) +  
    ...
    ylab(paste0(ylab)) +
    stat_bin2d(bins=25) + 
    xlim(0,max(x,na.rm=TRUE)) + ylim(0,max(x,na.rm=TRUE)) +
    scale_fill_gradientn(colours=r) + 
    geom_abline(intercept = 0,slope = 1,col='red3') +
    ggtitle(title)+
    geom_abline(slope=as.numeric(epsilon[1]),intercept = 0,color='blue3')+
    geom_abline(slope=as.numeric(epsilon[2]),intercept = 0,color='blue3',linetype="dashed")+
    geom_abline(slope=as.numeric(epsilon[3]),intercept = 0,color='blue3',linetype="dashed")
...
plotSI <- function(I, Rt1, Rt2, k, periods){
  ...
  legend('topleft',legend = variants,
         ...
         bty='n')
  abline(v = x[periods$intervals[c(3,4,5)]],col = 'darkorange3',lty=2)
