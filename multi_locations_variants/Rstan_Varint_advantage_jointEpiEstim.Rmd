...
n_loc <- ncol(data_incidence)/2
n_t <- nrow(data_incidence) # 100 time steps
n_v <- ncol(data_incidence)/2

priors <- default_priors()
# constant incidence 10 per day everywhere
incid <- array(NA, dim = c(n_t, n_loc, n_v))
incid[,,1] <- as.matrix(data_incidence[,c(1,2)])
incid[,,2] <- as.matrix(data_incidence[,c(3,4)])
...
R_init <- matrix(5, nrow = n_t, ncol = n_loc)
R_init[1, ] <- NA # no estimates of R on first time step
epsilon_init <- 5

res <- estimate_joint(epsilon_init, R_init,
                    incid, w, priors,
                    n_iter = 1000,
                    burnin = 100)
