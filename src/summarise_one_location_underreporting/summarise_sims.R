...
eps_summary_df <- pmap_dfr(
  x, function(rt_ref, epsilon, p_report, summary) {
    summary$rt_ref <- rt_ref
    summary$true_eps <- epsilon
    summary$p_report <- p_report
    summary
  }
)
...
## 3. by p_report
by_p_report <- split(eps_summary_df, eps_summary_df$p_report) %>%
  map_dfr(
    function(x) summarise_sims(na.omit(x)), .id = "p_report"
  )
## 4. by rt_ref
...
## 5. by rt_ref and p_report
by_rt_and_p_report <- split(
  eps_summary_df, list(eps_summary_df$rt_ref, eps_summary_df$p_report),
  sep = "_"
) %>%
  map_dfr(
...
by_all_vars <-  split(
  eps_summary_df,
  list(eps_summary_df$rt_ref, eps_summary_df$p_report, eps_summary_df$tmax),
  sep = "_"
) %>%
  map_dfr(
    function(x) summarise_sims(na.omit(x)), .id = "var"
...
ggsave(glue("figures/underreport_prop_in_95.png"), p)

## Proportion in 95% CrI by tmax
by_all_vars <-  split(
  eps_summary_df,
  list(eps_summary_df$rt_ref, eps_summary_df$true_eps, eps_summary_df$p_report, eps_summary_df$tmax),
  sep = "_"
) %>%
  map_dfr(
    function(x) summarise_sims(na.omit(x)), .id = "var"
...
by_all_vars <- tidyr::separate(by_all_vars, col = "var", into = c("rt_ref", "true_eps", "p_report", "tmax"), sep = "_")
by_all_vars$rt_ref <- as.factor(by_all_vars$rt_ref)
by_all_vars$p_report <- as.factor(by_all_vars$p_report)
...
x <- group_by(eps_err_summary_df, rt_ref, true_eps, p_report, tmax) %>%
  summarise(
