...
# select only the sims with following characteristics
# this is an exploratory scenario to demonstrate a single point, so do not need whole parameter grid
index <- which(param_grid$rt_ref == 1.1 &
                param_grid$epsilon == 1.5 &
                param_grid$si_mu_variant == 5.4)

simulated_incid <- simulated_incid[index]
