...
rows <- nrow(sim_params)
sim_params <- sim_params[seq_len(rows), ]
incid_init <- initial_incidence(n_loc = 2L)
...
# Introduce the variant in loc 2 20 days later than in loc 1
z <- c(rep(0, times = 20), simulated_incid[[1]][[1]][,,2])
z <- z[1:110]

simulated_incid[[1]][[1]][,2,2] <- z
