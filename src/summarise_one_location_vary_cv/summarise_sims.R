...
## 3. by si_mu
by_cv_var <- split(eps_summary_df, eps_summary_df$si_cv_variant) %>%
  map_dfr(
    function(x) summarise_sims(na.omit(x)), .id = "si_cv_variant"
  )
## 4. by rt_ref
...
by_rt_and_mu <- split(
  eps_summary_df, list(eps_summary_df$rt_ref, eps_summary_df$si_cv_variant),
  sep = "_"
) %>%
  map_dfr(
...
by_all_vars <-  split(
  eps_summary_df,
  list(eps_summary_df$rt_ref, eps_summary_df$si_cv_variant, eps_summary_df$tmax),
  sep = "_"
) %>%
  map_dfr(
    function(x) summarise_sims(na.omit(x)), .id = "var"
...
by_all_vars <- tidyr::separate(by_all_vars, col = "var", into = c("rt_ref", "si_cv_variant", "tmax"), sep = "_")
...
x <- group_by(eps_err_summary_df, rt_ref, si_cv_variant, tmax) %>%
  summarise(
...
x$si_label <- glue(
  "X {round(x$si_cv_variant / si_cv_ref, 1)}"
)
